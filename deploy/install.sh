...
if [ -f "$ENV_FILE" ]; then
	if yesno "$ENV_FILE exists. Reuse it and just (re)deploy?" y; then
		...
		DOMAIN="${MXS_DOMAIN:-}"; EXTRA_DOMAINS="${MXS_EXTRA_DOMAINS:-}"
		AI_MODEL="${MXS_AI_MODEL:-}"; AI_ENDPOINT="${MXS_AI_ENDPOINT:-}"
...
if [ "$REUSE_ENV" -eq 0 ]; then
	...
	ask EXTRA_DOMAINS "Additional hostnames for the same dashboard, space separated (optional)"
...
MXS_DOMAIN=$DOMAIN
MXS_EXTRA_DOMAINS=$EXTRA_DOMAINS
